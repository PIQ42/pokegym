        ...
        try:
            ...
            if self.use_screen_memory:
                ...
                result = np.concatenate(
                    (screen_data[..., :2], extended_mmap, status_channel, events_channel),  #screen_data has more than 2 channels
                    axis=2
                )
            else:
                result = np.concatenate(
    ...
    def calculate_reward(self):
        if self.badges == 0:
           self.reward = self.reward_scale * (
            ...
            self.reward = self.reward_scale * (
            + 10 #BASE REWARD NOT ACTUALLY NEEDED HERE SINCE NOTHING IS REMOVED 
            + self.event_reward * 5
            + self.caught_pokemon_reward * 5
            + self.moves_obtained_reward
            + self.level_reward
