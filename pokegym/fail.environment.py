...
class Base:
    ...
    def render(self):
        # (144, 160, 3)
        image = self.pyboy.botsupport_manager().screen().screen_image()
        image_array = np.array(image)  # Convert PIL image to a numpy array
        # If you need only one channel (e.g., green channel), you can do:
        game_pixels_render = np.expand_dims(image_array[:, :, 1], axis=-1)  # Green channel
