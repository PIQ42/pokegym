...
class Environment(Base):
    ...
    def calculate_reward(self):
        if self.badges == 0: #max(self.get_party_levels()  consider force levling before this point we can also force party size
			if ram_map.read_bit(self.game, 0xD7F3, 2) != 1: #First Viridian Forest Trainer, Needed because we return to route 2 after forest
                ...
                + (ram_map.map_n(self.game) == 13) * 3# Route 2
                + (ram_map.map_n(self.game) == 50) * 4# Southgate
                + (ram_map.map_n(self.game) == 51) * 5# Forest
            #+ (ram_map.map_n(self.game) == 47) * 6# Northgate
            #Pewter City and Gym
                + self.event_reward * 5
			...
			elif ram_map.read_bit(self.game, 0xD7F3, 2) == 1:
                self.reward = self.reward_scale * (
                + (ram_map.map_n(self.game) == 12) # Route 1
                + (ram_map.map_n(self.game) == 1) * 2 #Veridian and Buildings
                + (ram_map.map_n(self.game) == 41) * 2 
            ...
            self.reward = self.reward_scale * (
            + 10 #Offset Previous Location based Rewards
            + self.event_reward * 5
            + self.caught_pokemon_reward * 5
            + self.moves_obtained_reward
            + self.level_reward
            ...
            if self.bill_state < 1: #Objective is get SS Anne Ticket
                self.reward = self.reward_scale * (
                + 20 #NOTHING REMOVED THIS STILL SHOULDN'T BE NEEDED
                + self.event_reward * 5
                ...
                + 30 #NOTHING REMOVED THIS STILL SHOULDN'T BE NEEDED
                + self.event_reward * 5
                + self.bill_capt_reward
                + self.caught_pokemon_reward * 5
                + self.moves_obtained_reward
